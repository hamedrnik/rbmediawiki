...
# * api_url: the url of the api, as in http://en.wikipedia.org if not specified, it will be guessed from the lang+family values or the server
    ...
    def login(password = nil)
        ...
        confirm_token_result = make_request(post_me,login_result)

        @config['lgusername']  = confirm_token_result['login']['lgusername']
        @config['lguserid']    = confirm_token_result['login']['lguserid']
        @config['lgtoken'] 	   = confirm_token_result['login']['lgtoken']
        @config['_session']    = confirm_token_result['login']['sessionid']
        @config['cookieprefix']= confirm_token_result['login']['cookieprefix']
    ...
    def make_request(post_this, login_result=nil)
        ...
        if @config['logged']
          ...
          cookies = login_result['login']['cookieprefix'] + '_session=' + login_result['login']['sessionid']
        else
          cookies = ""
        end
